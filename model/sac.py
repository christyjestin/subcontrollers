...
class SAC:
    ...
    def __init__(self, env_fn, actor_critic = core.MLPActorCritic, ac_kwargs = dict(), seed = 0, 
                 ...
                 update_every = 50, num_test_episodes = 10, max_episode_length = 1000, logger_kwargs = dict(), save_freq = 1):
    ...
    def compute_loss_q(self, data):
        observation, action, reward, next_observation = data['obs'], data['act'], data['rew'], data['obs2']
        done = data['done']
    ...
    def test_agent(self):
        for _ in range(self.num_test_episodes):
            observation, done, episode_return, episode_length = self.test_env.reset(), False, 0, 0
            for _ in range(self.max_episode_length):
                # Take deterministic actions at test time 
                observation, reward, done, _ = self.test_env.step(self.get_action(observation, deterministic = True))
    ...
    def run(self):
        ...
        for t in range(total_steps):
            ...
            # Ignore the "done" signal if it comes from hitting the time horizon (that is, when
            # it's an artificial terminal signal that isn't based on the agent's state)
...
if __name__ == '__main__':
    ...
    SAC(lambda: gym.make(args.env), actor_critic = core.MLPActorCritic, gamma = args.gamma, seed = args.seed, 
